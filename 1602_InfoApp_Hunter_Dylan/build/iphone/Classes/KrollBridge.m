...
NSString * k1602_InfoApp_Hunter_Dylan$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1602_InfoApp_Hunter_DylanObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1602_infoapp_hunter_dylan);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1602_infoapp_hunter_dylan gc];
}
...
{
	// create k1602_InfoApp_Hunter_Dylan global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1602_InfoApp_Hunter_Dylan" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1602_infoapp_hunter_dylan = [[k1602_InfoApp_Hunter_DylanObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1602_infoapp_hunter_dylan];
	
	NSString *_k1602_infoapp_hunter_dylanNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1602_infoapp_hunter_dylanNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1602_infoapp_hunter_dylan valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1602_InfoApp_Hunter_Dylan$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1602_infoapp_hunter_dylan.kroll" 
