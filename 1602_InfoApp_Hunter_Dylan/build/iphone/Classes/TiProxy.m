...
//Rare exceptions to indicate a bug in the _k1602_infoapp_hunter_dylan code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k1602_infoapp_hunter_dylan = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k1602_infoapp_hunter_dylan]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k1602_infoapp_hunter_dylan.module"
