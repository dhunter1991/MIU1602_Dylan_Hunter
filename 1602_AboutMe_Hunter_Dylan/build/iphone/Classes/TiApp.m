...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1602_AboutMe_Hunter_DylanUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1602_AboutMe_Hunter_DylanUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1602_AboutMe_Hunter_Dylan app using the node.js CLI
