...
//Rare exceptions to indicate a bug in the _practice_dylan code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_practice_dylan = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_practice_dylan]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.practice_dylan.module"
